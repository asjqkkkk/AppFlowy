...
async fn test_search_no_hits() -> Result<()> {
  ...
  let results = db
    .search(&workspace_id, &[], &query, 1, EmbeddingDimension::Dim768)
    .await?;
  assert!(
    results.is_empty(),
...
async fn test_select_multiple_oids() -> Result<()> {
  ...
  let map = db
    .select_collabs_fragment_ids(&[oid1.clone(), oid2.clone()], EmbeddingDimension::Dim768)
    .await?;
  assert_eq!(map.len(), 2);
...
async fn test_object_ids_handling() -> Result<()> {
  ...
  let result = db
    .select_collabs_fragment_ids(&all_oids, EmbeddingDimension::Dim768)
    .await?;
